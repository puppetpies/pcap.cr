...
module Pcap
  class Packet
    ...
    {% for method in %w(ip_hl ip_id ip_len ip_proto ip_sum ip_tos ip_ttl ip_v) %}
      def {{ method.id }}
        ip_header.{{ method.id }}
      end
    {% end %}
    {% for method in %w(tcp_data_len tcp_dst tcp_doff tcp_hlen tcp_seq tcp_sum tcp_src tcp_win) %} 
      def {{ method.id }}
